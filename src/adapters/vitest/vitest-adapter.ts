...
export class VitestAdapter implements TestAdapter {
  ...
  public async detect(context: AdapterContext): Promise<boolean> {
    ...
    try {
      ...
      return !!deps.vitest || fs.existsSync(path.join(context.projectRoot, 'vitest.config.ts'));
    } catch {
      return false;
    }
  }
  ...
  public async discover(context: AdapterContext): Promise<TestDiscoveryResult> {
    ...
    return { runner: 'vitest', testFiles };
  }
  ...
  public async run(request: TestRunRequest, context: AdapterContext): Promise<TestRunResult> {
    ...
    const outputFile = path.join(guard.getVeloProveDirectory(), 'cache', 'vitest-output.json');
    args.push(`--outputFile=${outputFile}`);

    // Prefer local Vitest binary via node (avoids Windows npx.cmd spawn EINVAL)
    const localVitest = [
      path.join(context.projectRoot, 'node_modules', 'vitest', 'vitest.mjs'),
      path.join(context.projectRoot, 'node_modules', 'vitest', 'dist', 'cli.js')
    ].find((p) => fs.existsSync(p));

    const res = localVitest
      ? await runner.run(process.execPath, [localVitest, ...args], {
          cwd: context.projectRoot,
          timeoutMs: request.timeoutMs || DEFAULT_EXECUTION_POLICY.timeouts.processMs
        })
      : await runner.run('npx', ['vitest', ...args], {
          cwd: context.projectRoot,
          timeoutMs: request.timeoutMs || DEFAULT_EXECUTION_POLICY.timeouts.processMs
        });
    ...
    if (fs.existsSync(outputFile)) {
      try {
        ...
        // fallback to stdout parse if json reading fails
      }
    }

    // If no test results were parsed from file, parse from stdout/stderr
    if (testResults.length === 0) {
      const isPass = res.exitCode === 0;
      testResults.push({
        id: `vitest-suite`,
        title: 'Vitest Execution',
