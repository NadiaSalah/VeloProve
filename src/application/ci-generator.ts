...
jobs:
  veloprove-verification:
    ...
    steps:
      ...
      - name: Project Twin + Drift (PARTIAL MVP)
        run: |
          npx veloprove twin update --with-impact --with-drift || npx veloprove twin build --with-impact --with-drift
          npx veloprove drift --json

      - name: Run Test Suites
        run: npx veloprove test --affected || npx veloprove test
